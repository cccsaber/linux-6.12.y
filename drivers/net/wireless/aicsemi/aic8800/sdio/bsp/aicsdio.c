...
static int aicbsp_platform_power_on(void)
{
	...
	aicbsp_unreg_sdio_notify();
#if !defined(CONFIG_PLATFORM_ALLWINNER) && !defined(CONFIG_PLATFORM_AMLOGIC) && \
	!defined(CONFIG_PLATFORM_ROCKCHIP) && !defined(CONFIG_PLATFORM_ROCKCHIP2)
	/*
	 * On RK3528 the SDIO functions are already enumerated at boot, so a
	 * separate power/card-detect callback is optional for bring-up. If no
	 * platform power backend is compiled in, continue and let the real SDIO
	 * driver bind the existing device.
	 */
	sdio_dbg("%s no platform power backend, continue with existing sdio device\n",
		 __func__);
	return 0;
#endif
#ifdef CONFIG_PLATFORM_ALLWINNER
